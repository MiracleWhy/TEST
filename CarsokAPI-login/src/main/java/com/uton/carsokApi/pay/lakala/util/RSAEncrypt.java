...
public class RSAEncrypt implements SecurityOperator {
	 ...
	 * Exception
	 */
	public boolean verifySign(String message, String signStr, String key) throws Exception {

		return this.verify(KeyTool.loadPublicKey(key), message.getBytes("UTF-8"), Base64.decode(signStr));
	}
	...
	public byte[] sign(RSAPrivateKey privateKey, byte[] cipherData) throws Exception {
		// 用私钥对信息生成数字签名
		if (privateKey == null) {
			throw new Exception("解密私钥为空, 请设置");
		}
	...
	public boolean verify(RSAPublicKey publicKey, byte[] cipherData, byte[] signText) throws Exception {
		try {
