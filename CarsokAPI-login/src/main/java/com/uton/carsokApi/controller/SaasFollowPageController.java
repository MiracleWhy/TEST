 ...
 */
@Controller
public class SaasFollowPageController {

    private final static Logger logger = Logger.getLogger(SaasFollowPageController.class);
    ...
    @RequestMapping("getFollowPage")
    @ResponseBody
    public BaseResult getPotCusPage(HttpServletRequest request, ReportDateRequest vo){
        try {
            ...
            if(acount.getSubPhone()!=null){
                ...
                if(authority.equals("Staff")){//员工权限
                    ...
                    FollowResponse followResponse=followService.getFollowPage(acount,childAccounts,vo);
                    return new BaseResult(ErrorCode.SuccessRetCode, ErrorCode.SuccessRetInfo,followResponse);
                }
            }
            //管理者权限子账号+主账号
            ...
            logger.error("获取跟进报表页 getFollowPage  error:", e);
